    ...
    def update_style(self):
        ...
        self.setStyleSheet(f"""
            QPushButton {{
                ...
                padding: 10px; font-weight: bold; border-radius: 4px; text-align: left;
            }}
            QPushButton:hover {{
...
class LMSPlugin(QWidget):
    ...
    def get_path(self, code, section, filename):
        ...
        target_dir = os.path.join(root, code, cat)
    ...
    def download(self, url, filename, section, code):
        save_path, category = self.get_path(code, section, filename)
    ...
    def batch_download(self, contents, section, code):
        ...
        for i, item in enumerate(contents):
            ...
            self.download(u, n, section, code))

    # --- DEEP FETCH (Assignment PDFs) ---
    def deep_fetch(self, url, cid, code):
        self.active_tasks += 1;
        self.dl_screen.start("SCANNING...")
        try:
            cmid = int(url.split("id=")[-1])
        except:
            self.active_tasks -= 1; return
                ...
                for a in c.get('assignments', []):
                    if a['cmid'] == cmid:
                        for att in a.get('introattachments', []):
                            self.download(att['fileurl'], att['filename'], "Assignments", code)
                            found = True
    ...
    def setup_login_view(self):
        ...
        self.u_in = QLineEdit();
        self.u_in.setPlaceholderText("ID");
        self.u_in.setFixedWidth(300)
        self.p_in = QLineEdit();
        self.p_in.setPlaceholderText("PASSWORD");
        self.p_in.setEchoMode(QLineEdit.Password);
        self.p_in.setFixedWidth(300)
        style = "background:#111; color:white; padding:8px; border:1px solid #333;"
        self.u_in.setStyleSheet(style);
        self.p_in.setStyleSheet(style)

        btn = AnimatedButton("CONNECT", primary=True);
        btn.setFixedWidth(300)
        btn.clicked.connect(self.perform_login)

        l.addWidget(QLabel("LMS GATEWAY", styleSheet=f"color:{ACCENT}; font-size:20px; font-weight:bold;"));
        l.addSpacing(20)
        l.addWidget(self.u_in);
        l.addWidget(self.p_in);
        l.addWidget(btn)
    ...
    def load_and_verify(self):
        if os.path.exists(CONFIG_FILE):
            try:
                ...
                pass
            if self.token:
                self.fetch_courses(); self.stack.setCurrentIndex(1)
            else:
            ...
            for m in sec['modules']:
                ...
                if m['modname'] == 'resource' and 'contents' in m:
                    ...
                    btn_dl.clicked.connect(
                        lambda chk=False, u=f['fileurl'], n=f['filename'], s=sec['name'], c=code: self.download(u, n, s,
                                                                                                                c))
                elif m['modname'] == 'folder' and 'contents' in m:
                    btn_dl.setText("BATCH")
                    btn_dl.clicked.connect(
                        lambda chk=False, i=m['contents'], s=sec['name'], c=code: self.batch_download(i, s, c))
    ...
    def fetch_deadlines(self):
        ...
        def on_dl(res):
            ...
            for e in res['data'].get('events', []):
                code = e.get('course', {}).get('fullname', "GENERIC").split()[0]
                ...
                info = QVBoxLayout()
                info.addWidget(
                    QLabel(f"[{code}] {e['name']}", styleSheet="color:white; font-weight:bold; border:none;"))
                ...
                btn.clicked.connect(lambda chk=False, u=e['url'], i=cid, c=code: self.deep_fetch(u, i, c))
